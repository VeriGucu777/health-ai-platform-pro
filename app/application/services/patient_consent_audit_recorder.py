"""Fail-open patient consent audit recording."""
...
async def record_patient_consent_audit_event(
    ...
    *,
    ...
    """Persist consent management audit without PHI in metadata."""
    if audit_service is None or audit_context is None:
        return

    safe_metadata = dict(metadata or {})
    for forbidden in ("email", "first_name", "last_name", "phone", "notes", "body", "text"):
        safe_metadata.pop(forbidden, None)

    entry = AuditRecordInput(
        resource_type=AuditResourceType.PATIENT_CONSENT,
        action=action,
        outcome=outcome,
        ...
        resource_id=consent_id or patient_id,
    ...
    try:
        ...
        logger.exception(
            "Failed to persist patient consent audit event",
            extra={
