"""Fail-open clinical timeline audit recording helpers."""
...
async def record_clinical_timeline_audit_event(
    ...
    *,
    ...
    """Persist a clinical timeline view audit event without breaking the HTTP response."""
    if audit_service is None or audit_context is None:
        return

    entry = AuditRecordInput(
        resource_type=AuditResourceType.PATIENT_CLINICAL_TIMELINE,
    ...
    try:
        ...
        logger.exception(
            "Failed to persist clinical timeline audit event",
            extra={
