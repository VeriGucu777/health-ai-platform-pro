"""Fail-open health report export audit recording helpers."""
...
HEALTH_SUMMARY_PDF_METADATA = {"report": "health_summary", "format": "pdf"}


async def record_health_report_export_audit_event(
    ...
    *,
    ...
    """Persist a health report PDF export audit event without breaking the HTTP response."""
    if audit_service is None or audit_context is None:
        return

    entry = AuditRecordInput(
        resource_type=AuditResourceType.HEALTH_REPORT,
        ...
        metadata=dict(HEALTH_SUMMARY_PDF_METADATA),
    )
    ...
    try:
        ...
        logger.exception(
            "Failed to persist health report export audit event",
            extra={
                "audit_action": AuditAction.EXPORT.value,
